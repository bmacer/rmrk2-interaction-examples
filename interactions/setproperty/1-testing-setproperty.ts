...
const aliceSetsPropertyOnColor = async (alices_nft_block_number: number) => {
    console.log("Step 2: Alice creates NFT");
    try {
        const api = await ApiPromise.create({ provider: wsProvider });
        const keys = getKeys();
        ...
        // const minted = nft.mint();
        const setproperty_remark = nft.setproperty("color", "red");
        console.log(setproperty_remark);

        const tx = api.tx.system.remark(setproperty_remark);
        const { block } = await sendAndFinalize(tx, keys[0]);
        console.log("Chunky NFT minted at block: ", block);
        return block;
    } catch (error: any) {
...
const run = async () => {
    ...
    await aliceSetsPropertyOnColor(alices_nft_id);
    process.exit(0);
}
