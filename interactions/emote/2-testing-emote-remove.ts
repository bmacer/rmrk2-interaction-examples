...
const bobRemovesEmoteOnAlicesNft = async (alices_nft_block_number: number) => {
    console.log("Step 4: Bob Removes Emote on Alice's NFT");
    try {
        const api = await ApiPromise.create({ provider: wsProvider });
        const keys = getKeys();
        const nft = new NFT({
...
const run = async () => {
    ...
    await bobRemovesEmoteOnAlicesNft(alices_nft_block_number);
    process.exit(0);
}
