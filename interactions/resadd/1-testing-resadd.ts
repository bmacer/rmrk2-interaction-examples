...
const aliceAddsResourceToNft = async (alice_nft_block_number: number) => {
    ...
    try {
        ...
        const resadd_remark = alices_nft.resadd({ id: "alices_resource_id" });
...
const run = async () => {
    ...
    await aliceAddsResourceToNft(alices_nft_id);
    process.exit(0);
}
