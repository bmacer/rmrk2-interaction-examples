...
const aliceMintsAnNft = async (sn: number) => {
    console.log("Step 2-3: Alice MINTs two NFTs");
    try {
        const api = await ApiPromise.create({ provider: wsProvider });
        const keys = getKeys();
        const nft = new NFT({
...
const aliceBurnsParentNft = async (parent_nft_number: number) => {
    console.log("Step 5: Alice burns parent NFT.  Grandchild should also be BURNed");
    try {
        const api = await ApiPromise.create({ provider: wsProvider });
        const keys = getKeys();

        const parent_nft = new NFT({
            block: parent_nft_number,
            collection: "ALICES_COLLECTION",
            symbol: `ALICES_NFT_1`,
            transferable: 1,
            sn: "1",
        ...
        const burn_remark = parent_nft.burn();
...
const run = async () => {
    ...
    await aliceBurnsParentNft(alices_first_nft_id);
    process.exit(0);
}
